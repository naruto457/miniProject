...
def sendMail(sendTo, msg, sub):
    ...
    for person in sendTo:
        message = f"""From: From Person {EMAIL_ID}
        To: To Person {person}
        Subject: {sub}

        {msg}
        """
        mail.sendmail(EMAIL_ID, person, message)  # send part
...
def composeMail():
    ...
    Returns:
        None: None
    """
    SpeakText("Mention the gmail ID of the persons to whom you want to send a mail. Email IDs should be separated with the word, AND. Speak")
    ...
    SpeakText("The mail will be send to " +
              (' and '.join([str(elem) for elem in emails])) + ". Confirm by saying YES or NO. Speak")
    ...
    SpeakText("Say your Subject. Speak")
    sub = speech_to_text()
...
def searchMail():
    ...
    mailBoxTarget = "INBOX"
    SpeakText("Where do you want to search ? Say 1 for Inbox. Say 2 for Sent Mailbox. Say 3 for Drafts. Say 4 for important mails. Say 5 for Spam. Say 6 for Starred Mails. Say 7 for Bin. Speak")
    cmb = speech_to_text()
    if cmb == "1" or cmb.lower() == "one":
        mailBoxTarget = "INBOX"
        SpeakText("Inbox selected.")
    ...
    SpeakText("Say 1 to search mails from a specific sender. Say 2 to search mail with respect to the subject of the mail. Speak")
    mailSearchChoice = speech_to_text()
    if mailSearchChoice == "1" or mailSearchChoice.lower() == "one":
        SpeakText("Please mention the sender email ID you want to search. Speak")
        ...
        SpeakText("Please mention the subject of the mail you want to search. Speak")
        searchSub = speech_to_text()
        status, messages = M.search(None, f'SUBJECT "{searchSub}"')
    else:
        SpeakText("Wrong choice. Performing default operation. Please mention the subject of the mail you want to search. Speak")
        searchSub = speech_to_text()
        status, messages = M.search(None, f'SUBJECT "{searchSub}"')
            ...
            if isinstance(response, tuple):
                ...
                if msg.is_multipart(): 
                    for part in msg.walk(): # iterate over email parts
                        ...
                        # PLAIN TEXT MAIL
                        if content_type == "text/plain" and "attachment" not in content_disposition:
                            SpeakText("Do you want to listen to the text content of the mail ? Please say YES or NO. Speak")
                            talkMSG1 = speech_to_text()
                            if "yes" in talkMSG1.lower():
                                SpeakText("The mail body contains the following:")
                                SpeakText(body)
